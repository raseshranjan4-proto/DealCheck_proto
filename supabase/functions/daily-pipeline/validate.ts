...
export function toDealRow(x: Extraction, sourceUrl: string): ValidationResult {
  ...
  return {
    ...
    row: {
      ...
      investors_list: Array.isArray(x.investors_list) ? x.investors_list : [],
      investors: Array.isArray(x.investors_list) && x.investors_list.length > 0 ? x.investors_list.join(", ") : null,
      region: x.region,
